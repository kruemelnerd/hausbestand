...
**Domain:** Mobile-first, floorplan-based building inventory web app (radiators/windows, photos, reports)
...
```bash
# Frontend core
npm install vue vue-router pinia zod tailwindcss @tailwindcss/vite

# Frontend dev
npm install -D typescript vite vitest @playwright/test eslint prettier
...
**If running local-first (current milestone):**
...
**If moving to production hardening (later milestone):**
...
1. **Keep contracts stable:** file service interface and REST API stay unchanged.
2. **Swap infrastructure adapters:**
   - local filesystem -> S3-compatible storage
   - local SMTP sink (Mailpit) -> managed SMTP provider
3. **Harden runtime config:** secure cookies, TLS-only endpoints, strict CORS/CSRF policy by deployment topology.
4. **Operational hardening:** managed backups for Postgres + object storage, retention policies, restore tests, and monitoring.
5. **Release discipline:** dependency update gates (Renovate age policy), SBOM/attestation in CI, and regression E2E on every PR.
...
---
*Stack research for: HeizInventur (local-first inventory capture web app)*
*Researched: 2026-05-20*
